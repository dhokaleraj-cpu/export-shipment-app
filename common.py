...
def get_user_allowed_product_ids(username=None):
    """Return product IDs user can access. Empty list means all products."""
    try:
        user = st.session_state.get("user") or {}
        username = username or user.get("username")
        role = user.get("role")
...
def product_access_filter_sql(product_alias="p"):
    """Return SQL fragment and params for current user's part-number access. Empty access list means all."""
    ids = get_user_allowed_product_ids()
    if not ids:
        return "", ()
    prefix = f"{product_alias}." if product_alias else ""
    placeholders = ",".join(["?"] * len(ids))
    return f" AND {prefix}id IN ({placeholders}) ", tuple(ids)

def filter_product_rows_for_current_user(rows, product_id_key="id"):
    """Filter product rows by current user's allowed part numbers. Empty access list means all."""
    ids = get_user_allowed_product_ids()
    if not ids:
        return rows
    allowed = set(ids)
    return [r for r in rows if int(r.get(product_id_key) or 0) in allowed]

def user_can_access_product_id(product_id, username=None):
    """True when user can access a product ID. Blank access setup means all allowed."""
    ids = get_user_allowed_product_ids(username=username)
    if not ids:
        return True
    try:
